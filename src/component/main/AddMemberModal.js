...
function AddMemberModal({ setOpenAddMember }) {
  ...
  const handleToggleCheckBox = (userChoosed) => {
    if(choosers.filter(chooser => chooser.uid===userChoosed.uid).length===1){
        setChoosers(choosers.filter(chooser => chooser.uid!==userChoosed.uid))
    }
    else{
  ...
  const handleRemoveChooser = (chooser) => {
      setChoosers(oldChoosers => oldChoosers.filter(oldChooser => oldChooser.uid!==chooser.uid))
  }
  ...
  const handleSubmit = () => {
    ...
    .doc(currentRoom.id);
    roomRef.update({
        members: [...choosers.map(chooser => chooser.uid),
      ...
      <div className="relative z-[1000] bg-white border w-[548px] shadow rounded-[8px]">
        ...
        <div className="">
            ...
            <div className="flex flex-col h-[300px]">
                <div className="">
                    ...
                    <ul key={index} className="flex flex-start px-[16px] py-[8px]">
                        ...
                        <li key={chooser.uid} className="flex flex-col items-center justify-content w-[16.66666%]">
                            ...
                            <div className="text-[.75rem] font-medium text-[#65676B] pt-[8px]">{getShortString(chooser.displayName,10)}</div>
                        </li>
                        ))}
                    </ul>
                    ))}
                ...
                <div className="overflow-y-auto">
                    <ul className="">
                        <h3 className="px-[16px] pb-[6px] pt-[16px] text-[.9375rem] text-[#65676B] leading-[1.3333] font-bold">Gợi ý</h3>
                        {results.map(result => (
                            <li key={result.uid} className="h-[52px] px-[8px] flex items-center w-full">
                            <div className="px-[8px] flex justify-between rounded-[8px] w-full cursor-pointer hover:bg-[#eee]">
                                <div className="flex items-center">
                                    ...
                                    <div className="text-[.9375rem] font-medium">{result.displayName}</div>
                                </div>
                                <div className="flex justify-center items-center">
                                    <input 
                                    type="checkbox"
                                    className="w-[20px] h-[20px]"
                                    onChange={() => handleToggleCheckBox(result)}
                                    checked={choosers.filter(chooser => chooser.uid===result.uid).length===1}
                                    />
                                </div>
                            </div>
                        </li>
            ...
            <div className="p-[16px] shadow">
                ...
                >
                    <div className="text-[.9375rem] font-semibold leading-[1.3333]">Thêm người</div>
                </div>
            </div>
        </div>
      </div>
